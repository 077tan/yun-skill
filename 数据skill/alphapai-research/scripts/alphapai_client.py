...
class AlphaPaiClient:
    ...
    def parse_sse(response) -> Dict:
        ...
        已加入 SSL 断连容错：若连接中途断开且已收到内容则返回已收内容，否则抛出异常。
        """
        ...
        try:
            ...
            # 网络断连：若已有内容则容错返回，否则重新抛出
            if answer:
                import sys
                print(f"\n[提示] 连接中途断开（{type(e).__name__}），已返回已收到的内容。\n", file=sys.stderr)
            else:
                raise
        # 刷新解码器残留字节
        try:
    ...
    def qa_text(
        ...
        if is_stream:
            import sys as _sys
            min_len = 200
            last_result = {"answer": "", "references": []}
            for attempt in range(1, 5):
                try:
                    result = self.parse_sse(
                        self._post("/alpha/open-api/v1/paipai/qa-text", payload, stream=True)
                    )
                    if len(result.get("answer", "")) >= min_len:
                        return result
                    if len(result.get("answer", "")) > len(last_result.get("answer", "")):
                        last_result = result
                    if attempt < 4:
                        print(f"[重试 {attempt}/4] 内容不完整，重新请求...", file=_sys.stderr)
                except Exception as e:
                    if attempt < 4:
                        print(f"[重试 {attempt}/4] 请求异常（{type(e).__name__}），重新请求...", file=_sys.stderr)
                    else:
    ...
    def stock_agent(
        ...
        # 最多重试 5 次，直到拿到足够长的完整内容（>3000字符视为完整）
        import sys as _sys
        min_complete_len = 3000
        last_result = {"answer": "", "references": []}
        for attempt in range(1, 6):
            try:
                ...
                if len(result.get("answer", "")) >= min_complete_len:
                    return result
                # 内容不足，保留最长的结果后重试
                if len(result.get("answer", "")) > len(last_result.get("answer", "")):
                    last_result = result
                if attempt < 5:
                    print(f"[重试 {attempt}/5] 内容不完整（{len(result.get('answer',''))} 字符），重新请求...", file=_sys.stderr)
            except Exception as e:
                if attempt < 5:
                    print(f"[重试 {attempt}/5] 请求异常（{type(e).__name__}），重新请求...", file=_sys.stderr)
                else:
                    if last_result.get("answer"):
                        return last_result
                    raise
